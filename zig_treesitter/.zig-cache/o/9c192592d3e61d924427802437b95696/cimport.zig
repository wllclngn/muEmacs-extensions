...
pub inline fn ptsname_r(arg___fd: c_int, __buf: [*c]u8, arg___buflen: usize) c_int {
    var __fd = arg___fd;
    _ = &__fd;
    _ = &__buf;
    var __buflen = arg___buflen;
    _ = &__buflen;
    return if (__builtin_object_size(@as(?*const anyopaque, @ptrCast(__buf)), @intFromBool(@as(c_int, 2) > @as(c_int, 1))) == @as(c_ulong, @bitCast(@as(c_long, -@as(c_int, 1))))) __ptsname_r_alias(__fd, __buf, __buflen) else __ptsname_r_chk(__fd, __buf, __buflen, __builtin_object_size(@as(?*const anyopaque, @ptrCast(__buf)), @intFromBool(@as(c_int, 2) > @as(c_int, 1))));
}
...
pub inline fn __fortify_clang_bosn_args(bos_fn: anytype, n: anytype, buf: anytype, div_1: anytype, complaint: anytype) @TypeOf("warning") {
    _ = &bos_fn;
    _ = &n;
    _ = &buf;
    _ = &div_1;
    _ = &complaint;
    return blk: {
        _ = __fortify_clang_bos_static_lt_impl(bos_fn(buf), n, div_1);
        _ = &complaint;
        break :blk "warning";
    };
}
