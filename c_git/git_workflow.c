...
static bool in_git_buffer(const char **out_name) {
    if (!api || !api->current_buffer || !api->buffer_name) return false;
...
static int git_init(struct uemacs_api *editor_api) {
    ...
    if (api->api_version < 3) {
        api->log_error("Git: Requires API v3 (event bus)");
        return -1;
    }
    ...
    api->log_info("Git workflow v3.0 loaded (buffer navigation, 12 commands)");
