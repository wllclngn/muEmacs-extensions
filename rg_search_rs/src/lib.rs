//! rg_search_rs - Rust ripgrep extension for μEmacs
//!
...
//!
//! Commands provided:
//! - rg-search-rs: Search for pattern in current directory
//! - rg-search-word-rs: Search for word under cursor
//!
...
use ffi::{UemacsApi, UemacsExtension};
...
extern "C" fn rg_init(api: *mut UemacsApi) -> c_int {
    ...
    unsafe {
        if let Some(register) = (*api).register_command {
            let cmd_search = CString::new("rg-search-rs").unwrap();
            let cmd_word = CString::new("rg-search-word-rs").unwrap();
        ...
        if let Some(on_key) = (*api).on_key {
            on_key(rg_key_hook);
        }

        // Log that we loaded
        if let Some(log_info) = (*api).log_info {
            let msg = CString::new("rg-search-rs: Rust extension loaded").unwrap();
            log_info(msg.as_ptr());
        }
    }
...
extern "C" fn rg_cleanup() {
    ...
    if !api.is_null() {
        unsafe {
            // Unregister key hook
            if let Some(off_key) = (*api).off_key {
                off_key(rg_key_hook);
            }
...
extern "C" fn rg_key_hook(key: c_int) -> c_int {
